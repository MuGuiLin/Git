...
要想统计某git分支的代码行数，先进入项目的根目录，在当前目录中打开Git工具（会显示当前所在分支名称），执行如下命令后，就会显示项目中指定统计代码加起来的总行数。

``` shell
git ls-files -- '*.html' '*.css' '*.js' '*.vue' ':!node_modules/**' | xargs grep -vE '^\s*$|^\s*//|^\s*/\*|^\s*\*|^\s*\*/' | wc -l
```

- 功能说明：统计当前 Git 项目中所有 .html、.css、.js、.vue 文件（排除 node_modules）中，非空且不是 JavaScript/CSS 风格注释的代码行数。

- 参数说明：

  1. **git ls-files -- '*.html' '*.css' '*.js' '*.vue' ':!node_modules/'**

     - git ls-files：列出 Git 跟踪的文件（即已加入版本控制的文件）。
     - --：表示选项结束，后面的内容被视为路径或模式。
     - '*.html' '*.css' '*.js' '*.vue'：匹配这些扩展名的文件。
     - :!node_modules/**：排除 node_modules/ 目录及其子目录下的所有文件（即使它们被 Git 跟踪了，通常不会，但这里做双重保险）。

     ✅ 结果：输出所有被 Git 跟踪的 .html、.css、.js、.vue 文件路径，但排除 node_modules 中的内容。

  2. **xargs grep -vE '^\s*$|^\s*//|^\s*/\*|^\s*\*|^\s*\*/'**

     - xargs：将上一步输出的文件列表作为参数传递给 grep。
     - grep -vE ...：
       - -v：反向匹配，即排除匹配的行。
       - -E：启用扩展正则表达式（Extended Regex）。
       - 正则表达式 '^\s*$|^\s*//|^\s*/\*|^\s*\*|^\s*\*/' 匹配以下内容：*
         - *^\s*$：空行（只包含空白字符或完全为空）。
         - ^\s*//：以 // 开头的单行注释（JavaScript/TypeScript 风格）。*
         - *^\s*/\*：以 /* 开头的多行注释起始行。
         - ^\s*\*：多行注释中的中间行（如  * 注释内容）。
         - ^\s*\*/：多行注释的结束行（如  */）。

     ✅ 结果：过滤掉所有空行和常见形式的注释行，保留“有效代码行”。

  3. **wc -l**

     - 统计上一步 `grep` 输出的总行数。
     - 即：**非空、非注释的有效代码行总数**（针对指定文件类型）。
...
> 例如：把test分支合并到beta分支**（注：要先切换到要合并的那个分支后，再进行合并）**
